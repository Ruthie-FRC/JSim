 ...
 */
JNIEXPORT jint JNICALL
Java_jsim_jni_JSimJNI_createAssembly
  (JNIEnv*, jclass, jlong world_handle)
{
  return static_cast<jint>(
      c_rsCreateAssembly(static_cast<uint64_t>(world_handle)));
}

/*
 * Class:     jsim_jni_JSimJNI
 * Method:    addRevoluteJoint
 * Signature: (JIIIDDD)I
 */
JNIEXPORT jint JNICALL
Java_jsim_jni_JSimJNI_addRevoluteJoint
  (JNIEnv*, jclass, jlong world_handle, jint assembly_index, jint body_a_idx,
   jint body_b_idx, jdouble axis_x, jdouble axis_y, jdouble axis_z)
{
  return static_cast<jint>(
      c_rsAddRevoluteJoint(static_cast<uint64_t>(world_handle), assembly_index,
                           body_a_idx, body_b_idx, axis_x, axis_y, axis_z));
}

/*
 * Class:     jsim_jni_JSimJNI
 * Method:    setJointLimits
 * Signature: (JIDD)I
 */
JNIEXPORT jint JNICALL
Java_jsim_jni_JSimJNI_setJointLimits
  (JNIEnv*, jclass, jlong world_handle, jint joint_id, jdouble min_angle_rad,
   jdouble max_angle_rad)
{
  return static_cast<jint>(
      c_rsSetJointLimits(static_cast<uint64_t>(world_handle), joint_id,
                         min_angle_rad, max_angle_rad));
}

/*
 * Class:     jsim_jni_JSimJNI
 * Method:    setJointMotorTarget
 * Signature: (JIDD)I
 */
JNIEXPORT jint JNICALL
Java_jsim_jni_JSimJNI_setJointMotorTarget
  (JNIEnv*, jclass, jlong world_handle, jint joint_id,
   jdouble target_velocity_radps, jdouble max_torque_nm)
{
  return static_cast<jint>(
      c_rsSetJointMotorTarget(static_cast<uint64_t>(world_handle), joint_id,
                              target_velocity_radps, max_torque_nm));
}
 ...
 */
JNIEXPORT jint JNICALL
Java_jsim_jni_JSimJNI_getJointAngle
  (JNIEnv* env, jclass, jlong world_handle, jint joint_id,
   jdoubleArray out_angle_rad)
{
  if (out_angle_rad == nullptr || env->GetArrayLength(out_angle_rad) < 1) {
    return -1;
  }

  double angle = 0.0;
  const int rc = c_rsGetJointAngle(static_cast<uint64_t>(world_handle), joint_id,
                                   &angle);
  if (rc != 0) {
    return rc;
  }

  jdouble values[1] = {angle};
  env->SetDoubleArrayRegion(out_angle_rad, 0, 1, values);
  return 0;
}

/*
 * Class:     jsim_jni_JSimJNI
 * Method:    getJointVelocity
 * Signature: (JI[D)I
 */
JNIEXPORT jint JNICALL
Java_jsim_jni_JSimJNI_getJointVelocity
  (JNIEnv* env, jclass, jlong world_handle, jint joint_id,
   jdoubleArray out_velocity_radps)
{
  if (out_velocity_radps == nullptr || env->GetArrayLength(out_velocity_radps) < 1) {
    return -1;
  }

  double velocity = 0.0;
  const int rc = c_rsGetJointVelocity(static_cast<uint64_t>(world_handle),
                                      joint_id, &velocity);
  if (rc != 0) {
    return rc;
  }

  jdouble values[1] = {velocity};
  env->SetDoubleArrayRegion(out_velocity_radps, 0, 1, values);
  return 0;
}
